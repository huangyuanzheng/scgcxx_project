...
<body>
	<div class="my_serach">
		...
		<select class="btn btn-default dropdown-toggle" id="my_status">
			<option  value="2">全部</option>
   			<option  value="0" selected="selected">待入库</option>
	...
	<script type="text/html" id="barDemo">
		{{#  if(d.store_status_name == '已包装'){ }}
		<a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="assign">入库</a>
		{{#  } }}
	</script>
	<script>
	...
	layui.use('table', function(){
		 ...
		 var tableIns = table.render({
		    ...
		    ,cols: [[ 
		          ...
		          ,{field: 'store_date', title:'入库日期', width:150, align:'center'}
		          ,{field: 'store_person', title:'入库登记人', width:130, align:'center'}
		  ...
		  table.on('tool(test)', function(obj){	//tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
			  ...
			  if(layEvent === 'assign'){
			  		layer.confirm('确认提交吗,提交后不能修改数据',
			  				...
			  				function(index){
			  					$.ajax({
			  						url:'doStore.do?product_id='+data.product_id+"&store_id="+data.store_id,
			  						cache: false
