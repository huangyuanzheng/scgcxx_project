...
<body>
	<div class="my_serach">
		...
		<select class="btn btn-default dropdown-toggle" id="my_status" name="process_status">
			<option  value="2">全部</option>
   			<option  value="0" selected="selected">待前处理</option>
   			<option value="1">已前处理</option>
   		</select>
	...
	<script type="text/html" id="barDemo">
		{{#  if(d.process_status_name ==='待前处理'){ }}
		<a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="assign">前处理</a>
		{{#  } }}
	</script>
	<script>
	...
	layui.use('table', function(){
		 ...
		 var tableIns = table.render({
		    ...
		    ,cols: [[ 
	          ...
	          ,{field: 'group_date', title: '分组日期', width: 130, align: 'center'}
	          ,{field: 'group_person', title: '分组人', width: 130, align: 'center'}
	          ,{field: 'pretreat_num', title:'前处理量', width:100, align:'center'}
	          ,{field: 'pretreat_date', title:'前处理日期', width:130, align:'center'}
	          ,{field: 'pretreat_person', title:'前处理人', width:130, align:'center'}
			  ...
			  if(layEvent === 'assign'){
				  layer.open({
					  ...
					  ,area:['600px', '700px']
				  	  ...
				  	  ,maxmin:true
				  	  ,anim:1
				  	  ,content:['toFinish.do?process_id='+data.process_id]
				  	  ,btn:['确认', '取消']
				  	  ,btnAlign: 'c'
				  	  ,yes:function(index, layero){
						var	nums = layer.getChildFrame('input[id=pretreat_num]', index);
						...
						if(nums.val() == ''){
							...
							layer.alert("前处理量不能小于零，请重新设置!!!",
					  		...
					  		layer.confirm('是否确认修改'+data.process_id+'号信息', function(index){
