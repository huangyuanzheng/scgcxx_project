...
<body>
		<div class="my_serach">
		编号：<input id="my_id" class="form-control" type="text" name="standard_id"  maxlength="8"  value="${param.standard_id}"/>
		名称：<input id="my_name" class="form-control" type="text" name="standard_name"  value="${param.standard_name}"/>
	...
	layui.use('table', function(){
		 ...
		 var tableIns = table.render({
		    ...
		    ,cols: [[ 
		      ...
		      ,{field:'standard_id', title: '编号', sort: true, align: 'center'}
		      ,{field:'standard_name', title: '名称',align: 'center'}
		  ...
		  table.on('tool(test)', function(obj){	//tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
			  ...
			  if(layEvent === 'del'){
				  layer.confirm('真的删除吗', function(index){
					 ...
					 layer.close(index);
					 $.ajax({
						url:'toDelete.do?standard_id='+data.standard_id
						,datatype:'json'
						,success:function(result){
							table.reload('itDemo');
							if(result.success){
								layer.msg(
										data.standard_id+result.msg,
										{
											icon:1
										})
							}
								...
								layer.alert(
										"编号"+data.standard_id+result.msg,
										{
											icon:5,
											anim:6
										})
				  ...
				  layer.open({
					  type:2
					  ,area:['500px', '600px']
				  	  ,title:'编号：'+data.standard_id
				  	  ,maxmin:true
				  	  ,anim:1
				  	  ,content:['toAlter.do?standard_id='+data.standard_id]
				  	  ,btn:['提交修改', '放弃修改']
				  	  ,btnAlign: 'c'
				  	  ,yes:function(index, layero){
						var name = layer.getChildFrame('input[id=pro_standard_name]', index);
						if(name.val() == "" || name.val() === null){
							layer.alert("请填写名称!!!",
					  		...
					  		layer.confirm('是否确认修改'+data.standard_id+"号信息", function(index){
		  ...
		  var $ = layui.$, active = {
			reload: function(){
				...
				table.reload('itDemo',{
					...
					,where:{
						standard_id:my_id.val(),
						standard_name:my_name.val()
					}
				});
			}
		  };
...
/* 		  var $ = layui.$, active ={
		...
		function add(){
			...
			layer.open({
				...
				,yes:function(index, layero){
					var name = layer.getChildFrame('input[id=pro_standard_name]', index);
					var id = layer.getChildFrame('input[id=pro_standard_id]', index);
