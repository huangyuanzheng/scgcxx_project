...
<body>
		<div class="my_serach">
		编号：<input id="my_id" class="form-control" type="text" name="ro_id"  maxlength="8"  value="${param.ro_id}"/>
		名称：<input id="my_name" class="form-control" type="text" name="ro_name"  value="${param.ro_name}"/>
	...
	<script type="text/html" id="barDemo">
		<a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="assign">权限分配</a>
	...
	layui.use('table', function(){
		 ...
		 var tableIns = table.render({
		    ...
		    ,cols: [[ 
		      ...
		      ,{field:'ro_id', title: '编号', sort: true, align: 'center'}
		      ,{field:'ro_name', title: '名称',align: 'center'}
		      ,{field:'ro_remark', title: '备注',align: 'center'}
			  ...
			  if(layEvent === 'assign'){
				  layer.open({
					  ...
					  ,area:['800px', '800px']
				  	  ...
				  	  ,maxmin:true
				  	  ,anim:1
				  	  ,content:['toAssign.do?ro_id='+data.ro_id]
				  });
				  ...
				  layer.confirm('真的删除吗', function(index){
					 ...
					 layer.close(index);
					 $.ajax({
						url:'toDelete.do?ro_id='+data.ro_id
						,datatype:'json'
						,success:function(result){
							table.reload('itDemo');
							if(result.success){
								layer.msg(
										data.ro_id+result.msg,
										{
											icon:1
										})
							}
				  ...
				  layer.open({
					  ...
					  ,area:['500px', '600px']
				  	  ...
				  	  ,maxmin:true
				  	  ,anim:1
				  	  ,content:['toAlter.do?ro_id='+data.ro_id]
				  	  ,btn:['提交修改', '放弃修改']
				  	  ,btnAlign: 'c'
				  	  ,yes:function(index, layero){
						var name = layer.getChildFrame('input[id=role_name]', index);
						if(name.val() == "" || name.val() === null){
							layer.alert("请填写名称!!!",
					  		...
					  		layer.confirm('是否确认修改'+data.ro_id+'号信息', function(index){
...
/* 		  var $ = layui.$, active ={
		...
		function add(){
			...
			layer.open({
				...
				,yes:function(index, layero){
					var name = layer.getChildFrame('input[id=role_name]', index);
					var id = layer.getChildFrame('input[id=ro_id]', index);
